...
public abstract class CommonPopupWindow<T extends ViewDataBinding> {
    ...
    public CommonPopupWindow(Context c, int layoutRes, int w, int h) {
        context = c;
        popBinding = DataBindingUtil.inflate(LayoutInflater.from(c), layoutRes, null, false);
        contentView = popBinding.getRoot();
