...
public class ReminderManager {
    ...
    private final void updateUnreadMessageNum(int unreadNum, boolean delta, int reminderId) {
        ...
        for (int i = 0; i < items.size(); i++) {
            ReminderItem item = items.valueAt(i);
            if (item == null) {
                continue;
            }
            ...
            totalNum += item.getUnread();

        }

        ReminderItem resultItem = new ReminderItem(0);
        resultItem.setUnread(totalNum);
        for (UnreadNumChangedCallback cb : unreadNumChangedCallbacks) {
            cb.onUnreadNumChanged(resultItem);
        }
    }
}
